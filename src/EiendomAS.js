...
const styles = `
  ...
  .ek-as-banner { background: var(--brg-pale); border-left: 3px solid var(--brg); padding: 12px 16px; font-size: 13px; color: var(--brg); line-height: 1.6; margin-bottom: 16px; }
`;
...
export default function EiendomAS() {
  ...
  return (
    <div className="ek-wrap">
      ...
      <div className="ek-as-banner">
        Kjøp via AS: Høyere egenkapitalkrav og ekstra kostnader, men bedre for portefølje med flere enheter og lavere utbytteskatt på reinvestert kapital.
      </div>
      ...
      <div className="ek-step">
        ...
        <div className="ek-two-col">
          <div className="ek-col">
            ...
            <span className={`ek-badge ${harRaad ? 'green' : 'red'}`}>{harRaad ? 'Du har råd' : 'Mangler ' + fmt(tot - kapital)}</span>
          </div>
          <div className="ek-col">
            ...
            <div className="ek-line"><span className="k">Skatt (22%)</span><span className="v" style={{color:'#8b2020'}}>-{fmt(skatt)}</span></div>
        ...
        <div className={`ek-verdict ${!harRaad ? 'red' : netto >= 0 ? 'green' : 'amber'}`}>
          {!harRaad
            ? `Du mangler ${fmt(tot - kapital)} for å gjennomføre kjøpet via AS.`
            : netto >= 0
      ...
      <EiendomPrognose
        ...
        rentePrivat={null}
        renteAS={rente}
        ekProsentAS={ekProsent}
        restKapitalPrivat={null}
        restKapitalAS={restKapital}
      />
    </div>
  );
}